...
- ✅ **Superintendent outreach machine** (`scripts/build_outreach_merge.py` + `scripts/send_outreach.py`): AskTED contacts + 3 frame-honest insights per district from committed artefacts; Resend API (UA header required — Cloudflare 403s bare urllib, NOT an auth failure); rails: --confirm GO, domain-verified check, TAG_POSTAL_ADDRESS (CAN-SPAM), idempotent sent-log, opt-out file, verify_targets() identity gate (deep link/site name/insights must all be the row's own district). **State: 71 real sends done** (50 pilot + 21 of halted wave 1); ~948 held pending owner go. Sent-log + opt-outs mirrored in Supabase (`sql/create_outreach_state.sql`, `scripts/sync_outreach_state.py`) — remote read failure fails CLOSED. Sender: `gus@ubntag.com` (ubntag.com verified in Resend; txisd.dev also verified via Vercel-DNS records added this session). RESEND_API_KEY + RESEND_FROM in Vercel env.
- ✅ **Watchdogs**: `.github/workflows/monitor.yml` (daily, no secrets: /health healthy+connected, cron-runs gaps, verify_live, check_freshness). `scripts/check_freshness.py` + `scripts/freshness_vintages.json` — bumping the vintage file is how a release is acknowledged. **First run found 3 stale layers: STAAR SY2026 out (equity uses 2024+2025), recapture 2026-27 out, TIGER2025 boundaries out (geomap on 2024)** — monitor will nag daily until ingested or acknowledged.
