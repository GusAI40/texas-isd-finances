...
async def cron_isd_intelligence(request: Request):
    ...
    try:
        ...
        print("ERROR: intelligence idempotency preflight failed "
    ...
    if existing:
        await core._record_cron_run(
            pool, "isd-intelligence", started, "skipped", 0,
            "already_ran_today"
        )
    ...
    def _run() -> dict:
        ...
        try:
            ...
            print("WARNING: TEA newsroom fetch failed "
                  f"({type(exc).__name__}); no exception detail logged")
        ...
        for query in queries[: int(os.getenv("ISD_MAX_QUERIES", "12"))]:
            try:
                ...
                print("WARNING: one intelligence feed failed "
                      f"({type(exc).__name__}); no exception detail logged")
    ...
    try:
        ...
        print("ERROR: intelligence research failed "
              f"({type(exc).__name__}); no exception detail logged")
        await core._record_cron_run(
            pool,
            "isd-intelligence",
        ...
        raise HTTPException(
            status_code=500,
            detail="The intelligence run failed before persistence.",
        ) from None
    ...
    try:
        ...
        print("ERROR: intelligence persistence failed "
              f"({type(exc).__name__}); no exception detail logged")
        await core._record_cron_run(
            pool,
            "isd-intelligence",
