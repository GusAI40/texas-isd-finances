...
def create_enrollment_vs_spending_scatter(data: List[Dict[str, Any]], year: int):
    ...
    # Add trendline: rolling mean of spending across districts ordered by enrollment
    trend = df.sort_values('enrollment')
    fig.add_trace(
        go.Scatter(x=trend['enrollment'],
                   y=trend['spend_per_student'].rolling(window=5, min_periods=1).mean(),
                   mode='lines',
                   name='Trend',
                   line=dict(color='red', dash='dash'))
    )

    fig.update_xaxes(type='log', title='Student Enrollment (log scale)')
    fig.update_yaxes(tickformat='$,.0f')
