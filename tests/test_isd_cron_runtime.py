...
def test_success_commits_briefing_and_review_queue_together(monkeypatch):
    ...
    assert run[3] == "ok"
    assert run[4] == 2
    # Public cron history accepts controlled codes only; row counts carry the
    # useful outcome without leaking dynamic text into a public log.
...
def test_concurrent_loser_skips_without_queue_writes(monkeypatch):
    ...
    assert run[5] == "already_ran_today"
...
def test_existing_run_skips_before_research(monkeypatch):
    ...
    assert calls["load"] == 0
    run = next(
        args for sql, args in pool.store["executed"] if "cron_runs" in sql
    )
    assert run[5] == "already_ran_today"
