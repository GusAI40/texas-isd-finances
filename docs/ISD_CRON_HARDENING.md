...
Cron-history `detail` is public aggregate telemetry. The handler therefore
writes only the controlled codes accepted by `src.api._public_cron_detail` and
logs exception classes without provider, driver, query, URL, or address text.
Counts such as `rows_written` carry the useful outcome.
